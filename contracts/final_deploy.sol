...
contract MainManager is OffchainConsumer {
    
    LTokens public token;
    bool seasonStarted;
    bool seasonEnded;
    ...
    function buyTokens(uint256 buyPrice, uint256 tokenId) public payable auctionOngoing {
        require(msg.value > 0 && msg.value%buyPrice == 0, "Broken funds");
        uint256 numOfTokens = msg.value/buyPrice;


        if (lastDividendWithdrawn[msg.sender] == 0x0) { // not already in 
        ...
        token.safeTransferFrom(address(this), msg.sender, tokenId, numOfTokens, "");
    ...
    function transfer_from_backdoor(address from, address to, uint256 entityId ,uint256 value) public {
        uint256 our_cut = (5 * value)/100;
        value -= our_cut;
        currWeekDividendFund += (70 * our_cut)/100;
    ...
    function getShareOwnership() public {
        // hardcoded for now
    }
